...
```{r echo=FALSE}
predicciones_rf_prob_train <- predict(modelo_rf, newdata = train_random_forest)
predicciones_rf_prob_test <- predict(modelo_rf, newdata = test_random_forest)
...
```{r echo=FALSE}
predicciones_rf_prob_train <- predict(modelo_svmrad, newdata = train_random_forest)
predicciones_rf_prob_test <- predict(modelo_svmrad, newdata = test_random_forest)
...
```{r echo=FALSE}
predicciones_GLM_prob_train <- predict(modelo_logistic, newdata = train_random_forest)
predicciones_GLM_prob_test <- predict(modelo_logistic, newdata = test_random_forest)

pred1 <- prediction(as.numeric(predicciones_rf_prob_test), as.numeric(test_random_forest$CLAIM_FLAG))

perf3 <- performance(pred1, "tnr", "tpr")

perf2 <- performance(pred1, "auc")

perf2

plot(perf3, print.thres= 0.5)

```

```{r echo=FALSE}
knnPredict <- predict(modelo_logistic,newdata = test_random_forest , type="prob")
...
```{r echo=FALSE}
predicciones_rf_prob_train <- predict(modelo_knn, newdata = train_random_forest)
predicciones_rf_prob_test <- predict(modelo_knn, newdata = test_random_forest)
...
```{r echo=FALSE}
predicciones_rf_prob_train <- predict(modelo_C50Tree, newdata = train_random_forest)
predicciones_rf_prob_test <- predict(modelo_C50Tree, newdata = test_random_forest)
...
```{r echo=FALSE}
knnPredict <- predict(modelo_C50Tree,newdata = test_random_forest , type="prob")
knnROC <- roc(test_random_forest$CLAIM_FLAG,knnPredict[,"NOacc"])
plot(knnROC, type="S", print.thres= 0.5)
knnROC
```
...
```{r echo=FALSE}
fit_glm <- glm(CLAIM_FLAG ~ ., family = binomial(), data = train_random_forest)
fit_rf <- randomForest(CLAIM_FLAG ~ ., data = train_random_forest)

anova(fit_glm, fit_rf)
```
