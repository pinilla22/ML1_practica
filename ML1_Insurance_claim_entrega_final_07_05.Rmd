...
4 - Entrenamiento e hiperparametros de los siguientes modelos(paquete utilizado caret) en los cuales hacemos 9 lanzamientos para cada combinación de parametros:
        ...
        - Random forest: Cambiando el numero de variables en cada arbol, el tamaño minimo del nodo y utilizando Gini para la pureza del nodo.
...
Vamos a realizar la explicación detallada de la opción ganadora explicada brevemente arriba.
...
Como hemos comentado el data set se compone de 27 variables y 10302 registros.
...
## Analisis de variables relevantes (INCLUIR AQUI EL RANDOM FOREST)
...
Finalmente nos quedamos con aquellas cuyo factor explicativo es mayor que 0.4
...
Para el analisis de componentes principales vamos a utilizar la BBDD sin las tres variables que nos predijeron los cluster que no las vamos a utilizar en la BBDD de los otros dos cluster que vamos a analizar con los modelos.
...
bins$EDUCATION %>%
  knitr::kable()
```
Finalmente nos quedamos con una BBDD con 12 variables con un balanceo de 32%(1) / 68%(0)
...
Ahora empezamos con modelos supervisados de clasificación. Para estos modelos partiremos de la BBDD anterior en la que hemos aplicado el WOE.
...
Finalmente para las redes bayesianas no hacemos pruebas con ningún parametro.
...
Primero vamos a analizar las medias de las valores que nos ha dado nuestro lanzamiento en train para los hiperparametros que han sido definitivamente el que mas accuracy tenian en cada modelo.
...
Vemos que el resultado de test es muy parecido al de train ganado levemente el knn con respecto al resto.

### Curva ROC

Ahora vamos a comparar las curvas ROC para ver como de diferentes son poder decidir definitivamente con que modelo nos quedamos.

Como vemos en la imagen podriamos decir que la curva ROC que mas nos convence es la del KNN aunque muy solapada con la del GLM.
...
Aplicamos entonces este set sobre el KNN obteniendo los siguientes resultados.
...
1 - Con unicamente tres variables de la persona (Si ha tenido siniestros anteriormente, Valor del vehiculo y Si conduce por ciudad) soy capaz de identificarte un porcentaje de siniestralidad para esa persona siendo uno de ellos practicamente un cero.

2 - Si quieres profundizar en los cluster que no son cero tendriamos un accuracy de un 75%.
